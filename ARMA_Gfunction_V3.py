...
class Arma11_Garch11_method():
	...
	def AG_data_process(self):
		...
		print('==============================================')
		print('I: ADF Stability test :')
		t = sm.tsa.stattools.adfuller(self.xt)  # ADF检验
		...
		if t[1] < 0.05:
			print('data stability is satisfied')
		else:
		   print('data stability is not satisfied, please consider higher order difference!')
		print(' ')

		print u'II：Pure randomness test '
		t = acorr_ljungbox(self.xt,lags = 1)
		#print(t[1])
		print ('p-value = %e'%t[1])
		if t[1] < 0.05:
			print('The series is non-pure Gaussian noise series')
		else:	
			print('pure Gaussian noise, not for ARMA fitting!!!')
		print('==============================================')
		...
		print('III: correlation test:')
		print('1. autocorrelations parameters(AC)')
		...
		print('')
		print('2. partial autocorrelations parameters(AC)')
		pacf = sm.tsa.pacf(self.xt, m) 
		out = np.c_[range(1,m+1), pacf[1:]]
		output = pd.DataFrame(out, columns=['lag', "PAC"])
		print(output)
		print('==============================================')
	...
	def Arma_Garch_fitting(self):
		...
		print(res.message)
		print('-----------------------------------------------')
		print('nu, lambda : %8.4f, %8.4f'%(para[0],para[1]))
		print('ARMA para  : a0, a1, a2: %8.4f, %8.4f %8.4f'%(para[2],para[3],para[3]))
		print('Garch para : c0, c1, c2: %8.4f, %8.4f %8.4f'%(para[5],para[6],para[7]))
		print('-----------------------------------------------')
